...
const { widget, activeUsers } = figma
const { useSyncedState, usePropertyMenu, AutoLayout, Text, SVG,  } = widget

function Widget() {
  const [count, setCount] = useSyncedState('count', 0)

  const [today, setToday] = useSyncedState('today', '');
  ...
  return (
    ...
    >
      ...
      <Text fontSize={32} width={510} horizontalAlignText={'center'}>
        {activeUsers.map((user) => {return user.name})}
      </Text>
      <Text fontSize={32} width={510} horizontalAlignText={'center'}>
        {today}
      </Text>
